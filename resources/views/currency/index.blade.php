...
<div class="row g-3 mb-4">
  <div class="col-6 col-md-3"><div class="card text-center"><div class="card-body py-3">
    ...
    <div class="fw-bold fs-5 mt-1">{{ $base }}</div><div class="text-muted small">Base Currency</div>
  </div></div></div>
  <div class="col-6 col-md-3"><div class="card text-center"><div class="card-body py-3">
    <i class="bi bi-bar-chart fs-3 text-primary"></i>
    <div class="fw-bold fs-5 mt-1">{{ count($ratesData['rates']??[]) }}</div><div class="text-muted small">Mata Uang Dipantau</div>
  </div></div></div>
  <div class="col-6 col-md-3"><div class="card text-center"><div class="card-body py-3">
    <i class="bi bi-arrow-up fs-3 text-danger"></i>
    <div class="fw-bold fs-5 mt-1">{{ $currencyRates->where('change_percent','>',0)->count() }}</div><div class="text-muted small">Menguat vs Kemarin</div>
  </div></div></div>
  <div class="col-6 col-md-3"><div class="card text-center"><div class="card-body py-3">
    <i class="bi bi-arrow-down fs-3 text-success"></i>
    <div class="fw-bold fs-5 mt-1">{{ $currencyRates->where('change_percent','<',0)->count() }}</div><div class="text-muted small">Melemah vs Kemarin</div>
  </div></div></div>
</div>
  ...
  <div class="col-lg-7">
    <div class="card">
      <div class="card-header d-flex justify-content-between">
        <span><i class="bi bi-table me-2 text-warning"></i>Kurs Real-Time (1 {{ $base }})</span>
        <small class="text-muted">{{ $ratesData['last_updated']??now()->format('d M Y H:i') }}
          @if(($ratesData['source']??'')==='database')<span class="badge bg-warning ms-1 text-dark" style="font-size:.62rem">Cache</span>@endif
        </small>
      </div>
        ...
        <div class="table-responsive" style="max-height:480px;overflow-y:auto">
          <table class="table table-hover mb-0">
            ...
            <tbody>
              ...
              @endphp
              <tr>
                <td><span class="badge bg-light text-dark border fw-bold">{{ $cur }}</span></td>
                <td class="small">{{ $cnt?$cnt->flag_emoji.' '.$cnt->name:'—' }}</td>
                <td class="text-end fw-semibold">{{ number_format($rate,$rate<1?6:2) }}</td>
                <td class="text-end {{ $tc }} small">{{ $ti }} {{ $ch!==null?number_format(abs($ch),2).'%':'—' }}</td>
                <td>@if($cnt)<a href="{{ route('currency.show',$cnt->code) }}" class="btn btn-xs btn-outline-primary" style="font-size:.7rem;padding:2px 8px">Grafik</a>@endif</td>
              </tr>
              @endforeach
            </tbody>
          </table>
